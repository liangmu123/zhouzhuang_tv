...
var hasIndex = EVM.cookie.getValue('iindex')
...
function getMessage(page, size, pos) {
    ...
    getDatas("article/articleList", params, function (err, res) {
        if (err) {
            console.log(err)
            return;
        }
        ...
        for (var i = 0; i < dataItems.length; i++) {
               ...
               str +='<div class="notice" data-id= '+dataItems[i].id+'>'
               str +='<div class="notice-title">'+dataItems[i].title+'</div>'
               str +='<div class="notice-time">'+dataItems[i].add_time+'</div></div>'
        }
...
var right = cmsIndex.modules[cmsIndex.moduleGuid()].module = (function () {
    var mod = {},
        ...
        listName = 'notice', //块
        page_size = 6,//列表总数
        length = 6, //listName实际数量
        now_page = 0,//当前页数
        total_page = 0; //总页数    
    mod.modName = 'ModelList1';
...
//       setTimeout(function () { timedCount(index) }, 100);
        }
    ...
    mod.action = function (directive) {
        ...
        switch (directive) {
            case 'back':
                ...
                EVM.cookie.setValue('videoIndex','');
                window.history.go(-1);
                break;
            case 'enter':
                // if (status == '1') {
                ...
                // }
                EVM.cookie.setValue('iindex',index);
                EVM.cookie.setValue('now_page',now_page);
             var id = evm.$('right').children[index].getAttribute('data-id');
             window.location.href = 'tongzhi-xq.html?id='+id;
                break;
        }
        return me;
    };
