...
function getSkills(page, page_size, pos) {
	// signd=getCookie("qianming");
	$.ajax({
		type: 'get',
		url: api + meetingList + "?tpe=" + thisType + "&device=tv" + '',
		dataType: "jsonp",
		jsonp: "callback",
		async: false,
		success: function(res) {
			// if (res.code == 10002) {
			//     window.location.href = 'login.html?backurl=myShare' + '&backs=-1';
			// }
			if(res.code == 0) {
				var mydata = res.data.items;
				skillData = [];
				for(var i = 0; i < mydata.length; i++) {
					skillData.push({
						"id": mydata[i].id,
						"name": mydata[i].title,
						// "skill": mydata[i].skill_title,
						"head": "http://slave.jscnwx.com/poster/movie/" + mydata[i].third_id + "/163x198_1.jpg",
						"state": mydata[i].status,
						"points": mydata[i].score,
						"video": mydata[i].video_url
					});
				}
				document.getElementById('page_show').style.display = "block";
				document.getElementById('total_page').innerText = res.data.totalpage;
				document.getElementById('now_page').innerText = res.data.curpage;

				right.index = pos == 'previous' ? page_size - 1 : 0;
				right.paint(right.index);
				if(pos != 'first') {
...
var left = cmsIndex.modules[cmsIndex.moduleGuid()].module = (function() {
	...
	mod.paint = function() {
		...
		for(var i = 0; i < items.length; i++) {
			...
			str += '<div class="leftBtn" id=' + containerId + '-list-' + i + ' type_style=' + items[i].type_style + ' type=' + items[i].type + ' leftindex=' + i + ' data-content=' + items[i].name + ">" + iname + '</div>';
		}
	...
	mod.cleanCss = function(number) {
		...
		elem = evm.$(containerId + '-list-' + index);
		elem.className = elem.className + ' leftselect';
		return me;
	}
	...
	mod.moveH = function(directive) {
		...
		if(directive == "forwards") {
			me.cleanCss()
			leftindex = $('.leftselect')[0].getAttribute('leftindex');
			EVM.cookie.setValue('leftindex', index);
...
var right = cmsIndex.modules[cmsIndex.moduleGuid()].module = (function() {
	var mod = {},
		...
		//      index = 0, //右侧选中地址位置
		length = skillData.length, //右侧可选中总数实际数量 
		page_size = 8, //右侧列表总数
		now_page = 1, //当前页数
		total_page = 0, //总页数
		rowHeight = 2,
	...
	mod.paint = function(number) {
		...
		for(var i = 0; i < skillData.length; i++) {
			str += '<div class="skill" video=' + skillData[i].video + ' >';
	...
	mod.action = function(directive) {
		...
		switch(directive) {
			case 'enter':
				var mediaStr = $('.active')[0].getAttribute('video');
				//                     var thisId= evm.$('rightListBar').children[index].getAttribute('data-id');
				EVM.cookie.setValue('mediaStr', mediaStr)
				EVM.cookie.setValue('rightindex', index)
				window.location.href = 'play.html?mediaStr=' + mediaStr;
				//特殊处理: 判断栏目ID
				break;
			case 'back':
				//特殊处理: 判断栏目ID
